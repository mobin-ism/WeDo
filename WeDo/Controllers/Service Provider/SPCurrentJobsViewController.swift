...
class SPCurrentJobsViewController: UIViewController {
    ...
    lazy var menu: Menu = {
        let slideMenu = Menu()
        slideMenu.spCurrentJobsVC = self
        return slideMenu
    }()
    ...
    override func viewDidLayoutSubviews() {
        currentJobsButton.titleLabel?.font = UIFont(name: OPENSANS_BOLD, size: 12)
    }
    ...
    @objc private func navigationButtonTapped(_ sender: UIButton) {
        ...
        case 3:
            navigationController?.pushViewController(SPRequestedJobsViewController(), animated: false)
        default:
    ...
    public func selectedViewControllerFromMenu(indexNumber : Int) {
        if  UserDefaults.standard.value(forKey: IS_SERVICE_PROVIDER) as! Bool == true {
            ...
            case 0:
                print("current view controller")
            case 1:
                print("current jobs view controller")
            case 2:
