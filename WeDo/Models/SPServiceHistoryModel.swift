...
struct SPServiceHistoryModel: Decodable {
    let data: SPJobHistory
    let code: Int
    let message: String
    let isSuccess: Bool
}

struct SPJobHistory: Decodable {
    let jobHistory: [SPJobs]
}

struct SPJobs: Decodable {
    let id: Int
    let title: String?
